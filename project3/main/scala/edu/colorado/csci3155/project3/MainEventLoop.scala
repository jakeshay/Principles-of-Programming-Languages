...
class MainEventLoop {
    ...
    */
    def processOneEvent (l: List[Entity]) (ev: Event): (List[Entity], List[Event]) = {
        val (lApplies, lDoesNotApply) = partition(l)(ev)
        val f = lApplies.foldLeft(List[Entity](), List[Event]()) { (ent, e) =>
            val (l1: List[Entity], l2: List[Event]) = e.handleEvent(this)(ev)
            (ent._1 ++ l1, ent._2 ++ l2)
        }
        val entitiesf = f._1
        val eventsf = f._2
        (lDoesNotApply ++ entitiesf, eventsf)
    }
   ...
   */
    final def performOneTransition: (List[Entity], List[Event]) = {
        val e = entities
        val f = events.foldLeft(List(e), List[Event]()) { (ent, e) =>
            val (t, fr) = processOneEvent(ent._1.last)(e)
            (ent._1 ++ List(t), ent._2 ++ fr)
        }
        (f._1.last, f._2)
    }
    ...
    final def runTrampoline( p: Panel, evLoopSleep: Int = 50, tickSpeed: Double = 0.4): Unit = {
        ...
        tick(tickSpeed)
        // Run the trampoline again -- this better get optimzed away or else we are in deep trouble.
