...
case class Move(dx: Int) extends Event {
    ...
    override def appliesTo(e: Entity): Boolean = e match {
        case DartShooter(x: Int, numDarts: Int) => true
        case _ => false
    }
}
...
case object BaloonEscaped extends Event {
    ...
    override def appliesTo(e: Entity): Boolean = e match {
        case ArcadeGameManager(gamePoints: Int) => true
        case _ => false
    }
}
    ...
    override def appliesTo(e: Entity): Boolean = {
        e match {
            case ArcadeBaloon(_, x1, y1, _) => {
                val newX = x - x1
                val newY = y - y1
                val expr = math.sqrt(math.pow(newX, 2) + math.pow(newY, 2))
                if (expr <= ScreenDimensions.baloonBurstDistance) {
                    true
                }
                else {
                    false
...
case class ArcadeGameManager(val gamePoints: Int) extends Entity {
    ...
    override def handleEvent(global: MainEventLoop)(e: Event): (List[Entity], List[Event]) = e match {
        case Tick(_) => {
            ...
            val rd = r.nextDouble
            if (rd < .02) {
                val manager = ArcadeGameManager(gamePoints)
                val baloon = new ArcadeBaloon(global.numEntities+1, 20, 500 + r.nextInt((600-500)+1), 4+r.nextInt(12-4)+1)
                (List(manager, baloon), List[Event]())
            }
            if (rd > .98) {
                val manager = ArcadeGameManager(gamePoints)
                val baloon = new ArcadeBaloon(global.numEntities+1, ScreenDimensions.width - 20, 500 + r.nextInt((600-500)+1), -12+r.nextInt(-4 +12)+1 )
                (List(manager, baloon), List[Event]())
            }
            else {(List(ArcadeGameManager(gamePoints)), List[Event]())}

        }
        case BaloonEscaped => (List(ArcadeGameManager(gamePoints+ScreenDimensions.pointsBaloonEscaped)), List[Event]()) //TODO: Add points given by ScreenDimensions.pointsBaloonEscaped, no new events
        case BaloonBurst => (List(ArcadeGameManager(gamePoints+ScreenDimensions.pointsBaloonBurst)), List[Event]()) //TODO: Add points given by ScreenDimensions.pointsBaloonBurst, no new events
        case Stop => (List[Entity](), List[Event]()) // TODO: Empty list of entities and events to stop the trampoline.
        case _ => (List(ArcadeGameManager(gamePoints)), List[Event]()) //TODO: Return this entity unchanged and the empty list of events.
    }
        ...
        e match {
            case Tick(delta) => {
                ...
                if (x1 >= ScreenDimensions.shooterMax || x1 <= 0) {
                    (List[Entity](), List(BaloonEscaped))
                }
                else {
                    (List(ArcadeBaloon(id, x1, y, driftX)), List[Event]())
                }
            }
            case Boom(_, _) => {
